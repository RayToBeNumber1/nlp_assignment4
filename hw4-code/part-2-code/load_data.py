...
class T5Dataset(Dataset):
    ...
    def process_data(self, data_folder, split, tokenizer):
        ...
        if split != 'test':
            self.decoder_outputs = tokenizer(
                self.sql_queries,
                add_special_tokens=True,
                padding=False,
                truncation=True,
    ...
    def __getitem__(self, idx):
        encoder_ids = torch.tensor(self.encoder_inputs['input_ids'][idx])
        encoder_mask = torch.tensor(self.encoder_inputs['attention_mask'][idx])
        
        if self.split != 'test':

            decoder_ids = torch.tensor(self.decoder_outputs['input_ids'][idx])
            return encoder_ids, encoder_mask, decoder_ids
        else:
